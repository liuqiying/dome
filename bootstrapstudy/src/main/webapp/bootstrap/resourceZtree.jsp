...
<script type="text/javascript">
    ...
    function add(){
        ...
        var resourceType= $("[name='addType']:checked").val();
        $.post(
            "<%=request.getContextPath()%>/resource/addResource.do",
            {"resourceName":name,"pid":pid,"url":addUrl,"resourceType":resourceType,},
            function(data){
                if(data.status==200){
                    //获取被选中的节点集合
                    var nodes = zTreeObj.getSelectedNodes();
                    var newNodes = {"resourceName":name,"pid":pid,"id":data.id,"resourceType":resourceType,"addUrl":addUrl};
    ...
    function update(){
        ...
        $.post(
            ...
            {"id":id,"resourceName":newName,"pid":pid},
            function(data){
                if(data.status==200){
                    //获取被选中的节点集合
                    var nodes = zTreeObj.getSelectedNodes();
